(function () {
    console.log('CNotes: [V16-Safety] Script file loaded.');
    ...
    // NEW: The safety feature to clean up old data
    function pruneOrphanedNotes() {
        console.log('CNotes: Pruning orphaned notes...');
        const context = SillyTavern.getContext();
        ...
        for (const charId of noteCharIds) {
            if (!existingCharIds.has(charId)) {
                console.log(`CNotes: Deleting notes for missing character: ${charId}`);
        ...
        if (prunedCount > 0) {
            console.log(`CNotes: Pruned ${prunedCount} orphaned note entries.`);
            saveNotes();
            SillyTavern.utility.showToast(`${prunedCount} orphaned note entries were cleaned up.`, "success");
        } else {
            console.log('CNotes: No orphaned notes found.');
        }
    }
    ...
    function refreshFoldersUI() { /* ... unchanged ... */ }
    function refreshNotesList(noteIndexToSelect = -1) { /* ... unchanged ... */ }
    function displaySelectedNote() { /* ... unchanged ... */ }
    function prepareNewNote() { /* ... unchanged ... */ }
    function handleFolderSelect() { /* ... unchanged ... */ }
    function handleSaveNote() { /* ... unchanged ... */ }
    function handleDeleteNote() { /* ... unchanged ... */ }
    function handleDeleteFolder() { /* ... unchanged ... */ }
    function ensureOnScreen() { /* ... unchanged ... */ }
    function createModal() { /* ... unchanged ... */ }
    ...
    try {
        ...
        // NEW: Run the cleanup routine on startup
        pruneOrphanedNotes();
    ...
    // --- UNCHANGED FUNCTIONS (for brevity, these are the same as the last version) ---
    // (This is just a comment, the actual functions below are needed in the real file)

    refreshFoldersUI = function() {
        const context = SillyTavern.getContext();
        if (!context || !context.characterId) { if (isModalOpen) closeModal(); return; }
    ...
    refreshNotesList = function(noteIndexToSelect = -1) {
    ...
    handleSaveNote = function() {
        ...
        if (!title) { SillyTavern.utility.showToast("Note title cannot be empty.", "error"); return; }
        let folderToSaveIn = folderNameInput.value.trim();
        if (!folderToSaveIn) { folderToSaveIn = '##root##'; }
        if (!characterNotesData[currentCharacterId][folderToSaveIn]) { characterNotesData[currentCharacterId][folderToSaveIn] = []; }
    ...
    handleDeleteFolder = function() {
        ...
        if (folderToDelete === '##root##') { SillyTavern.utility.showToast("Cannot delete the Root Folder.", "error"); return; }
        ...
        modal.innerHTML = `
            ...
            <div id="character-notes-content">
                <div class="cnotes-folder-row">
                    ...
                    <button id="character-notes-delete-folder" class="menu_button" title="Delete Selected Folder"><i class="fa-solid fa-trash-can"></i></button>
                </div>
